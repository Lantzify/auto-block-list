...
namespace AutoBlockList.Controllers
{
	...
	public class AutoBlockListApiController : UmbracoApiController
	{
		...
		public PagedResult<DisplayAutoBlockListContent> GetAllContentWithTinyMce(int page)
		{
			...
			{
				var contentIdsWithMacros = scope.Database.Fetch<int>(AutoBlockListConstants.SQL, new { propertyTypeIds = tinyMcePropertyTypeIds });
				...
				var items = pagedContentIds.Select(_contentService.GetById)
					.Where(x => x != null)
					.ToList();
